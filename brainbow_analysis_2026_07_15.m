...
for file_num = files_to_process
    ...
    [~, endIndex] = regexp(name, 'plane\d+', 'once');
    % Extrait de l'index 5 jusqu'à la fin de "planeX", ou renvoie 'suite2p' si non trouvé
    if ~isempty(endIndex) && strlength(name) >= 5
        identifier = extractBetween(name, 5, endIndex);
    else
        identifier = 'suite2p';
    end
