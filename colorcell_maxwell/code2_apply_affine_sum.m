...
path_tif_exvivo = "E:\Data\Aurelie\data\chroms\175\images\exvivosmall.tif"; %
...
% --- Sauvegarde du Volume 3D en Multi-page TIF (Format ImageJ) ---
...
% Métadonnées ImageJ pour forcer la lecture en (Z, C, Y, X)
total_images = nb_z_utiles * num_channels;
ij_desc = sprintf('ImageJ=1.53c\nimages=%d\nchannels=%d\nslices=%d\nhyperstack=true\nmode=composite', ...
    total_images, num_channels, nb_z_utiles);

t = Tiff(out_tif_volume, 'w');
tagstruct.ImageLength = outH;
tagstruct.ImageWidth = outW;
...
t.close();
disp('Volume 3D original sauvegardé avec métadonnées ImageJ !');
...
% --- Sauvegarde du Volume 3D sommé2 en Multi-page TIF (Format ImageJ) ---
...
t2 = Tiff(out_tif_volume_sum2, 'w');
tagstruct2.ImageLength = outH;
tagstruct2.ImageWidth = outW;
tagstruct2.Photometric = Tiff.Photometric.MinIsBlack;
tagstruct2.BitsPerSample = 16;
tagstruct2.SamplesPerPixel = 1;
...
for z = 1:nb_z_final
    for c = 1:num_channels
        if page == 1
            tagstruct2.ImageDescription = ij_desc_sum2;
        elseif isfield(tagstruct2, 'ImageDescription')
            tagstruct2 = rmfield(tagstruct2, 'ImageDescription');
        end
        
        t2.setTag(tagstruct2);
        t2.write(Volume_Warped_Sum2(:, :, z, c));
        
        if page < total_images_sum2
            t2.writeDirectory();
        end
        page = page + 1;
    end
...
disp('Volume 3D sommé2 sauvegardé avec métadonnées ImageJ !');
