...
figure('Name', 'Comparaison des Modèles', 'Position', [100, 100, 1200, 800]); % Ajout d'une figure pour les subplots

%% ================= 1. MODÈLE LINÉAIRE (LDA) OPTIMISÉ =================
% Entraînement initial pour trouver le meilleur Gamma (RDA)
opts = struct('CVPartition', cvp, 'ShowPlots', false, 'AcquisitionFunctionName', 'expected-improvement-plus');
MdlLin_opt = fitcdiscr(X, Ycat, 'DiscrimType', 'linear', ...
    'OptimizeHyperparameters', 'Gamma', ...
    'HyperparameterOptimizationOptions', opts);

% Évaluation du modèle optimisé avec la cross-validation pour récupérer les prédictions
CVMdlLin = crossval(MdlLin_opt, 'CVPartition', cvp);
accLin = 1 - kfoldLoss(CVMdlLin);
YPredLin = kfoldPredict(CVMdlLin);

% Accuracy sans les noirs
subplot(2, 2, 1);
confusionchart(Ycat, YPredLin, 'Title', sprintf('1. RDA (LDA Optimisée Gamma)\nAccuracy: %.1f%%', accLin*100));
...
fprintf('1. RDA (LDA opt Gamma)  : %.2f %%\n', accLin * 100);
...
% On utilise le modèle MdlLin_opt trouvé ci-dessus qui a déjà été entraîné
% sur 100% des données avec le meilleur Gamma. On le renomme juste MdlLin.
MdlLin = MdlLin_opt;
