function [certitude_max, scores, accuracy_pct,accuracy_pct_sans_noires, C_matrix,truth_labels] = test_model_all(MdlFinal, X, Ycat,cell_IDs,tableau,seuil_faible)
fprintf('\n\n====== ANALYSE DES PROBABILITÉS ET VÉRIFICATION GROUND TRUTH ======\n');
class_order = {'rouge','vert','bleu','jaune','magenta','cyan','blanc','noir'};

% =========================================================================
% AJOUT IMPORTANT : GESTION DES SCORES (SVM / Random Forest)
% =========================================================================
% Si c'est un SVM (ClassificationECOC), il faut convertir les scores en probabilités
if isa(MdlFinal, 'classreg.learning.classif.ClassificationECOC')
    % fitPosterior permet de calibrer les scores SVM en vraies probabilités (0 à 1)
    % On le fait dynamiquement si le modèle n'a pas déjà de probabilités.
    try
        [y_pred, ~, ~, posterior_probs] = predict(MdlFinal, X);
        if isempty(posterior_probs)
             MdlFinal = fitSVMPosterior(MdlFinal);
             [y_pred, scores] = predict(MdlFinal, X);
        else
             scores = posterior_probs;
        end
    catch
        % Si la conversion échoue, predict renverra quand même des valeurs
        [y_pred, scores] = predict(MdlFinal, X);
    end
else
    % Pour LDA (ClassificationDiscriminant) et Random Forest (ClassificationEnsemble)
    [y_pred, scores] = predict(MdlFinal, X);
end

% Sécurité supplémentaire : S'assurer que 'scores' est bien une probabilité (0 à 1)
% Si les scores ne somment pas à 1 (ou proche de 1), on les normalise manuellement
somme_scores = sum(abs(scores), 2);
if any(somme_scores > 1.01) || any(somme_scores < 0.99) || any(scores(:) < 0)
    % Transformer des scores bruts (ex: marge SVM) en probabilités
    scores = exp(scores) ./ sum(exp(scores), 2); % Softmax
end
